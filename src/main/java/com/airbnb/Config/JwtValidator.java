...
//		System.out.println("In JwtValidator Method ...");
		
		String path = request.getRequestURI();
		
		if(path.startsWith("/api/auth/signup") || path.startsWith("/api/auth/login")) {
			// Skip jwt validation for signup / login requests
		...
		if(token != null) {
//			System.out.println("JWT IN FILTER METHOD " + token);
...
//				System.out.println("Exception during token validation " + e.getMessage());
			...
			throw new BadCredentialsException("Jwt token Not Included in the request");
		}
